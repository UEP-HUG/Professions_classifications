...
if (file.exists(here("output", "dat_master_professions_long.rds"))) {
  dat_master_professions_long <- readRDS(here("output", "dat_master_professions_long.rds"))
} else {
...
start <- Sys.time() ; matches_jw <- fuzzyjoin::stringdist_left_join(
  ...
  relocate(master_profession_lem, .after = last_col()) ; end <- Sys.time() ; print(end-start)
...
remaining_bad_matches <- anti_join(bad_matches_jaccard, better_bad_matches_jaccard, by = join_by("participant_id" == "participant_id", "source" == "source")) |> select(-rowid) |> 
...
good_matches_jw_clean <- good_matches_jw |> select(participant_id, source, Name_fr, ISCO, dist_jw, dist_jaccard) |> 
  mutate(confidence = "High")
good_matches_jaccard_clean <- good_matches_jaccard |> select(participant_id, source, Name_fr, ISCO, dist_jw, dist_jaccard) |> 
  mutate(confidence = "High")
better_bad_matches_jaccard_clean <- better_bad_matches_jaccard |> select(participant_id, source, Name_fr, ISCO, dist_jw, dist_jaccard) |> 
...
occup_matched <- left_join(occup, all_good_matches, by = join_by("participant_id" == "participant_id", "source" == "source")) |> 
  mutate(
    ISCO_full = ISCO,
...
occup_final <- occup_matched |> 
  ...
  select(participant_id, source, date_soumission, master_profession_original, Name_fr, Occupation_label, complementary_info, management, dist_jw, dist_jaccard, confidence, ISCO, ISCO_full) |> 
